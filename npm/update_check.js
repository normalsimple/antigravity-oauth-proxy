...
const DEFAULT_PACKAGE_NAME = "antigravity-oauth-proxy";
const DEFAULT_UPDATE_COMMAND = "npm install -g antigravity-oauth-proxy";
...
function isDisabled() {
  return (
    process.env.ANTIGRAVITY_PROXY_NO_UPDATE_NOTICE === "1" ||
    process.env.ANTIGRAVITY_PROXY_NO_UPDATE_NOTICE === "true" ||
...
function fetchLatestVersion(packageName, timeoutMs, signal) {
  ...
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { "User-Agent": `${CACHE_DIR_NAME}-update-check` } }, (res) => {
